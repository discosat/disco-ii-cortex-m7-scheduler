...
if [ "$#" -ne 1 -a "$#" -ne 2 ] || ! [ "$1" == "ram" -o "$1" == "ram_secure" -o "$1" == "flash"  -o "$1" == "flash_secure" ] ; then
  echo "Usage: $0 target"
...
if [ "$#" -eq 2 ] ; then
	if [ "$1" == "ram" ]; then
		$IMGUTIL --combine base_addr=0x1FFD0000 ivt_offset=0x1000 config_offset=0x400 config_file=qcb.bin app_offset=0x2000 app_file=$2.bin ofile=$2.img > /dev/null
	elif [ "$1" == "ram_secure" ]; then
	    $IMGUTIL --combine base_addr=0xFFD0000 ivt_offset=0x1000 config_offset=0x400 config_file=qcb.bin has_csf=1 app_offset=0x2000 app_file=$2.bin ofile=$2.img > /dev/null
	elif [ "$1" == "flash" ]; then
		$IMGUTIL --combine base_addr=0x04000000 ivt_offset=0x1000 config_offset=0x400 config_file=qcb.bin app_offset=0x2000 app_file=$2.bin ofile=$2.img > /dev/null
	elif [ "$1" == "flash_secure" ]; then
	    $IMGUTIL --combine base_addr=0x04000000 ivt_offset=0x1000 config_offset=0x400 config_file=qcb.bin has_csf=1 app_offset=0x2000 app_file=$2.bin ofile=$2.img > /dev/null
	fi
fi
